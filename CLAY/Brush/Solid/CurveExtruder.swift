...
struct CurveExtruder {
    ...
    /// Topology of each triangle strip in the extruded solid. It is static and determined by the number of points on the shape.
    ...
    @MainActor private mutating func reallocateMeshIfNeeded() throws -> Bool {
        ...
        //the shape is instantiated at each sample
        ...
        //then create the newMesh
        let newMesh = try Self.makeLowLevelMesh(vertexCapacity: newVertexCapacity, indexCapacity: newIndexCapacity)
        
        //the topology is fixed, it needs to write to the index buffer once
        newMesh.withUnsafeMutableIndices { buffer in
            //fill the index with triangleFanCapacity copies of the array topology offset for each sample
    ...
    init(shape: [SIMD2<Float>]) {
        ...
        /*
         triangle fan lists each vertex in a shape once for each ring, except for vertex 0 of the shape which
    ...
    /// Updates the `LowLevelMesh` maintained by this CurveExtruder. This applies pending calls to `append` or `removeLast` to the `LowLevelMesh`
    /// - Returns: A `LowLevelMesh` if a new mesh had to be allocated or `nil` otherwise
    @MainActor
