...
func evaluateHermiteCurve<T: HermiteInterpolant>(_ point0: HermiteControlPoint<T>, _ point1: HermiteControlPoint<T>, parameter: Float) -> T {
    ...
    //the basis vectors of an Hermite curve
...
///   - parameter: The parameter value with which to interpolate the control points `point0`
///         (which corresponds to `parameter == 0`) and `point1` (which corresponds to `parameter == 1`).
func evaluateHermiteCurveDerivative<T: HermiteInterpolant>(_ point0: HermiteControlPoint<T>, _ point1: HermiteControlPoint<T>, parameter: Float) -> T {
...
func evaluateCatmullRomSpline<T: HermiteInterpolant, S: RandomAccessCollection<T>>(spline: S, parameter: Float, derivative: Bool) -> T where S.Index == Int {
...
func subdivideCatmullRomSpline<T: HermiteInterpolant, S: RandomAccessCollection<T>>(spline: S, range entireRange: ClosedRange<Float>, flatness: Float, maximumSubdivisionDepth: Int = 8) -> [Float] where S.Index == Int {
